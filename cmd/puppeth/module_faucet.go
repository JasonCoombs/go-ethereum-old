...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/VentureCurrency                                                            && \
	(cd $GOPATH/src/github.com/VentureCurrency && git clone --depth=1 https://github.com/VentureCurrency/go-ethereum) && \
  go build -v github.com/VentureCurrency/go-ethereum/cmd/faucet                                              && \
